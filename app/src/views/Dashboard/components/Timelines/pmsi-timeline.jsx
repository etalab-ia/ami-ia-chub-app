...
import { PmsiModal } from "../Modals/pmsi-modal";
import { PmsiDocument } from "../Documents/pmsi-document.jsx";
import { getTooltipValue } from "./utils.jsx";

const PmsiTimelineBase = class PmsiTimeline extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
    ...
    this.onButtonDiagnosis = this.onButtonDiagnosis.bind(this);
		...
		this.props.displayTooltip(
			<div style={{fontSize: "0.85em", paddingBottom: 4}}>
				<div style={{fontSize: "1.2em", fontWeight: "bold", marginBottom: 4}}>
                    {e.get("pmsis")[0].type} ({getTooltipValue(e.get("pmsis"))})
				</div>
				...
				{(a => a && <ul style={{margin: 0, padding: 0, listStyle: "none inside"}}>
					{a.slice(0, 10).map((a, i) => <li key={i}>
						...
						{a.type}
						<span style={{fontWeight: "bold", marginLeft: 8}}>Diagnostiques: </span>
  ...
  onButtonDiagnosis(title, data) {
    ...
    this.state.openDocumentCallback(
      title,
      <PmsiDocument pmsiDocDiag={data}></PmsiDocument>
    );
  }
    ...
    return (
      <>
			<ChartContainer
				...
				<ChartRow key="PMSI" height="16" axisMargin={0}>
					<Charts>
						<EventChart
							series={timeSeries}
							size={18}
        ...
        <PmsiModal
          visible={openModal}
          data={modalData || {pmsis: []}}
          handleCloseModal={this.handleCloseModal}
          onButtonDiagnosis={this.onButtonDiagnosis}
        ></PmsiModal>
      </>
    );
  }
...
export const PmsiTimeline = sizeAware(connect(undefined, {displayTooltip, removeTooltip})(PmsiTimelineBase));
